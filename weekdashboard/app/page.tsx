...
const routines = [
  ...
  { id: "room", icon: "🧸", name: "Uklidit pokoj", detail: "hračky na své místo" },
...
function subscribeToProgress(onStoreChange: () => void) {
  ...
  return () => window.removeEventListener("weekdashboard-progress-changed", onStoreChange);
}
...
export default function Home() {
  ...
  const completed: CompletionState = progressSnapshot ? JSON.parse(progressSnapshot) : {};
  ...
  function toggleRoutine(day: string, routine: string) {
    ...
    window.localStorage.setItem("weekdashboard-progress", JSON.stringify(nextProgress));
    ...
    <main className="dashboard-shell">
      <section className="dashboard" aria-labelledby="dashboard-title">
        <header className="dashboard-header">
          ...
          <div className="weekly-score" aria-label={`${completedCount} z ${totalCount} úkolů hotovo`}>
            <span>Hotovo</span>
        ...
        <section className="week-grid" aria-label="Úkoly pro tento týden">
          <div className="grid-corner" aria-hidden="true">Úkoly</div>
          {days.map((day) => (
            <div className="day-heading" key={day.key} title={day.fullLabel}>{day.label}</div>
          ))}
          ...
          {routines.map((routine) => (
            <div className="routine-row" key={routine.id}>
              ...
              {days.map((day) => {
                ...
                return (
                  <label className="check-cell" key={key} title={`${routine.name}: ${day.fullLabel}`}>
                    <input
                      type="checkbox"
                      checked={isCompleted}
